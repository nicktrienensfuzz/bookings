...
function getAllIntervals(date) {
  ...
  let intervalStartTime = selectedDate.clone().set({'h': 7, 'm': 0, 's': 0});
  ...
  if (selectedDate.startOf('day').isSame(moment().utcOffset("-07:00").startOf('day'))) {
    ...
    if ((moment().hour() > 9) && (moment().hour() < 17)) {
      intervalStartTime = moment().add(30 - moment().minute() % 30, 'm').subtract(moment().seconds(), 's');
    } 
